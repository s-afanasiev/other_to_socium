...
function CategoriesDirectPass(catStorage, catTypes, oneCatBypass){
    ...
    this.run=(CATS_SRC, CATS_DST)=>{
        ...
        fs.writeFileSync("MEDIA_REPORT.txt", _report);
        return this;
    }
}
...
function BothProjectdataMediaIds(finalRevealExtraMediaIds, dstProjMedArrayShadow){
    ...
    this.run=(CATS_SRC, src_cat_key, CATS_DST, MEDIADATA_SRC, MEDIADATA_DST)=>{
        ...
        this.dstProjMedArrayShadow = this.dstProjMedArrayShadow.instance().run(dst_proj_media_ids_shadow, MEDIADATA_DST);
    ...
    this.run=(src_proj_ids, dst_proj_ids, MEDIADATA_SRC, MEDIADATA_DST, dstProjMedArrayShadow, src_cat_key, CATS_DST)=>{
        ...
        if(src_proj_ids.length > 0){
            src_proj_ids.forEach(src_one_media_id=>{
                _report += this.ifProjArrayIntersectMedfileIds.run(
...
function DstProjMedArrayShadow(){
    ...
    this.run=(dst_proj_media_ids_shadow, MEDIADATA_DST)=>{
    ...
    this.extra_report=()=>{
        ...
        this._report += "Excess links in dst project: ids=" + this.dst_proj_media_ids_shadow +"; links="+links + os.EOL;
        return this._report;
    }
    this.del=(ids)=>{
        if(Array.isArray(this.dst_proj_media_ids_shadow)){
            ids.forEach(id=>{
                ...
                if(id_index > -1){
                    this.dst_proj_media_ids_shadow = this.dst_proj_media_ids_shadow.splice(id_index, 1);
                }
            })
        }else{
...
function IfProjArrayIntersectMedfileIds(dstMedfileIdsByLink){
    ...
    this.run=(src_one_media_id, dst_proj_ids, MEDIADATA_SRC, MEDIADATA_DST, dstProjMedArrayShadow, src_cat_key, CATS_DST)=>{
        ...
        if(this.intersected(_dst_medfile_ids)){
            ...
            _report += "No Link " + this.link() + " in dst project at category:" + src_cat_key + os.EOL;
        }
        return this;
    }
}
